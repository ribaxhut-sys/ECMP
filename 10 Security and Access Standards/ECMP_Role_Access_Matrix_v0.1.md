...
> Nama file `..._v0.1.md` sengaja dipertahankan agar tautan lintas-dokumen stabil; **versi konten otoritatif = field `Version` di header** (saat ini 0.2).
...
|---|---|---|---|---|---|---|
| `cases:assign` | ❌ | ✅ Planned | ❌ | ❌ | `POST /v1/cases/{caseId}/assign` (API-003) | FRD-002 FR-003, BR-002 |
| `cases:status` | ❌ | ❌ | ✅ Planned | ❌ | `POST /v1/cases/{caseId}/status` (API-004) | FRD-002 FR-004, BR-001 |
...
- Nama permission transisi status mengikuti endpoint `POST /v1/cases/{caseId}/status`; bila FRD-002/OpenAPI memfinalkan nama lain (mis. `cases:transition`), revisi berikut menyelaraskan.

"""CompleteTicket command (TASK-062)."""
...
class CompleteTicketCommand:
    ticket_id: uuid.UUID


class CompleteTicketHandler:
    def __init__(
        self,
    ...
    def handle(self, command: CompleteTicketCommand) -> QueueTicketDto:
        ...
        updated = self._domain.transition_ticket(
            ticket, QueueTicketStatus.COMPLETED
        )
        self._state.replace_ticket(updated)
        return QueueTicketDto.from_domain(updated)


__all__ = ["CompleteTicketCommand", "CompleteTicketHandler"]

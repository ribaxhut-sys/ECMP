"""Case inbox receipts (Cabang unread) + Pusat unclaimed-queue count.
...
class CaseInboxRepository:
    ...
    def count_pusat_queue(self) -> int:
        """Distinct complaints that still need a Pusat handler."""
        unclaimed = {
            _norm(cid)
            for cid in self._session.scalars(
                select(CmCaseORM.complaint_id).where(
                    CmCaseORM.escalated_to_pusat.is_(True),
                    or_(
                        CmCaseORM.handling_claimed_by.is_(None),
                        CmCaseORM.handling_claimed_by == "",
                    ),
                    ~CmCaseORM.status.in_(_TERMINAL_CASE),
                )
            )
            if _norm(cid)
        }
        waiting = {
            ...
            for row in self._session.scalars(
                select(CmBatch1ComplaintORM).where(
                    CmBatch1ComplaintORM.intake_disposition == _ESCALATE_APPROVED,
                    CmBatch1ComplaintORM.hq_accepted_at.is_(None),
                    CmBatch1ComplaintORM.status != _CLOSED_COMPLAINT,
                )
            )
        }
        return len(unclaimed | waiting)
...
def safe_work_badge_counts(
    ...
    *,
    ...
    try:
        ...
        queue = repo.count_pusat_queue() if actor_is_pusat else 0
        return unread, queue
    except Exception:

...
def test_by_branch_preserves_desc_order() -> None:
    branch_a = uuid.uuid4()
    branch_b = uuid.uuid4()
    repo = MagicMock()
    repo.count_by_branch.return_value = [
        (branch_a, "BR-A", "Alpha", 10),
        (branch_b, "BR-B", "Beta", 3),
        (None, None, None, 1),
    ]

    result = ReportService(repo).by_branch()

    assert [item.total for item in result] == [10, 3, 1]
    assert result[0].branch_id == branch_a
    assert result[2].branch_id is None

> **Historical / SUPERSEDED** (2026-07-30): Archived under PROGRAM-BOARD-004 C-1 / PROGRAM-GOVERNANCE-001. Canonical current ADR-015 is `ECMP_ADR_015_Enterprise_Identity_Contract_v1.3.md` (Accepted with Conditions — BR-010). Content retained unchanged for history.
# ECMP_ADR_015_Enterprise_Identity_Contract_v1.2
...
- ADR Status: Proposed
- Board Disposition: Needs Revision (PROGRAM-ADR-002 BR-006) â€” coordinated revision package PROGRAM-ENTERPRISE-001; resubmit with ADR-014. Board Resolution traceability: `18 Architecture Governance/ECMP_PROGRAM_ADR_002_Board_Resolutions_v1.0.md`
- Date: 2026-07-30
...
- Package: PROGRAM-ENTERPRISE-001 â€” submit only with ADR-014
- Identity Contract Version: **1.0** (document revision 1.2 does not change contract major/minor claim set)
...
- Missing attributes may be handled inconsistently â€” sometimes defaulted, sometimes ignored â€” producing insecure or non-deterministic access outcomes.
...
- Runtime enforcement of this contract applies to **Mode B (Enterprise)**. Mode A (Standalone) is outside runtime enforcement but must not contradict these ownership rules when Mode B is later enabled (see Â§11).
...
| Role-Permission Matrix SoT (Role, Permission, Role-Permission, User-Role) | **Core Platform (ADR-008)** | Consume / enforce via Core Platform â€” **not** a second SoT |
...
Absence of any required claim is a contract failure and must result in access denial (see Â§10).

No additional claim is required by this ADR unless a later version of this contract marks it required.

Note: ADR-012 uses a different historical claim vocabulary (for example `sub`, `roles[]`, `orgUnitId`). That vocabulary is not the SoT for Mode B enterprise identity claims. Any conveyance mapping from ADR-012-era models to this contract requires Board-disposed relationship reconciliation and a follow-up binding decision â€” it must not silently redefine this table.
...
- Uniqueness of email, if enforced anywhere, is an Enterprise Platform concern â€” not an ECMP identity-key rule.
...
- Exact enumerated value set may be refined by a follow-up ADR without changing the claim name, provided compatibility rules in Â§11 are respected.
...
2. **The identity contract is versioned independently from implementation** â€” application release versions, deployment modes, and protocol bindings must not silently redefine the contract.
3. Document revision of this ADR (for example 1.2) may clarify ownership/relationship language without changing contract version **1.0**, provided required claims, key rules, and fail-closed behavior are unchanged.
4. Contract changes are classified as:
   - **Compatible** â€” add optional claims; clarify semantics without changing required meaning; add non-breaking enumerated values with documented defaults/ignore behavior.
   - **Breaking** â€” add/remove/rename required claims; change required claim meaning; change identity-key rules; change fail-closed behavior.
...
- **Unknown claims are ignored** â€” they do not cause denial by themselves, and they do not become implicitly required.

Fail closed means: **no access**, not â€œbest effort with incomplete identity.â€
...
Enterprise Platform Trust Domain
  ...
  - Owns organization structure referenced by identity
        ...
        v
ECMP Trust Domain
  - Authenticates trust in the enterprise presentation mechanism (protocol ADR â€” out of scope here)
...
- Existing claim models in earlier auth ADRs must be reconciled (see Â§17 and ADR Relationship).

## 15. Risks

| ID | Risk | Impact | Likelihood | Mitigation |
|---|---|---|---|---|
| R-01 | Producers omit required claims in some environments | High â€” widespread access denial or unsafe bypass pressure | Medium | Fail closed; contract conformance checks in architecture review and integration verification |
| R-02 | Teams continue using email as a de-facto key | High â€” identity collisions / wrong-subject access | Medium | Explicit prohibition in Â§7; review of data models and join paths |
| R-03 | Protocol ADRs redefine claims without updating this contract | High â€” dual sources of truth | Medium | This ADR is SoT for claim contract; protocol ADRs must map to it |
| R-04 | Optional claims silently become treated as required in code | Medium â€” brittle integrations | Medium | Compatibility rules + code review against required claim list |
| R-05 | Unreconciled overlap with ADR-007 / ADR-012 claim models | High â€” conflicting implementation targets | High until Board disposition | Identical Relationship proposals in ADR-014 + this ADR; follow-up binding ADR |
| R-06 | Local projection drift if reconciliation is never implemented | Medium â€” stale org/status decisions | Medium | Lifecycle Â§9 requires reconcile capability; scheduling left to follow-up |
| R-07 | ADR-014 retains a competing claim list | High â€” dual SoT | Low after PHASE-2 package | ADR-014 points to this ADR as claim SoT |

## 16. Alternatives Considered

### Alternative A â€” Leave identity attributes informal under ADR-014 only
...
### Alternative B â€” Define identity contract inside a protocol/binding ADR
...
### Alternative C â€” ECMP defines and owns the identity schema, Enterprise Platform adapts
...
### Alternative D â€” Canonical Enterprise Identity Contract owned at the platform boundary (this ADR)
...
1. **Identity conveyance / protocol binding** â€” how identity claims are presented to ECMP (without changing claim ownership or semantics defined here). OD-FE-002 remains downstream and is **not** closed by this ADR.
2. **Enterprise Entitlement representation** â€” how module entitlement is expressed and evaluated relative to identity (complements ADR-014 gate; does not redefine the gate).
3. **Organization Synchronization** â€” how organization/branch/department references remain resolvable for authorization (dependency already recorded in ADR-014).
4. **Identity reconciliation schedule and authority path** â€” operational cadence for Â§9 Reconcile.
5. **`employment_status` enumeration normative set** â€” exact allowed values and mapping to activate/deactivate.
6. **ADR relationship reconciliation** â€” Architecture Board disposition of ADR-007 and ADR-012 relative to Mode B / ADR-014 / this contract (see Relationship table proposals; no self-supersession).
7. **Contract version negotiation** â€” optional runtime assertion of `identity_contract_version` and multi-version support windows.
...
| ADR | Relationship |
|---|---|
| ADR-002 | **Consistent** â€” ECMP is not SoR for data it does not own; identity follows the same principle. |
| ADR-008 | **Complementary / Constrains** â€” enterprise identity â‰  Role-Permission Matrix SoT. Role-Permission Matrix SoT remains Core Platform. |
| ADR-014 | **Complementary** â€” ADR-014 decides AuthN ownership, Business Module boundary, and Entitlement Gate; this ADR is the Identity Contract SoT those decisions require. Submit as one package. |
| ADR-007 | **Proposed disposition for Board (not executed):** Mode Aâ€“scoped applicability. Slice/target auth model remains valid for Mode A; under Mode B, AuthN ownership follows ADR-014 and identity claims follow this ADR. Status remains **Relationship Pending** until Board confirms. |
...
- [ ] Confirm ADR-014 references this ADR as the canonical Enterprise Identity Contract SoT (done in PHASE-2 draft; preserve after acceptance)
- [ ] Board disposition of ADR-007 / ADR-012 relationships (proposals above â€” identical to ADR-014)
...
| Version | Date | Notes |
|---|---|---|
| 1.0 | 2026-07-29 | Initial Proposed â€” Identity Contract SoT |

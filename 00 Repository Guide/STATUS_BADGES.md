...
5. **Lifecycle ADR/DEC vs badge:** ADR/DEC memiliki lifecycle sendiri (`Proposed → Accepted → Superseded/Deprecated`) yang ditulis di **badan dokumen** (baris `ADR Status:` / `Status:`). Tabel metadata tetap memakai empat nilai badge di atas; lifecycle `Accepted` dipetakan ke badge `🟢 Approved`. Badan dokumen = otoritatif untuk lifecycle; tabel metadata = otoritatif untuk badge repo.

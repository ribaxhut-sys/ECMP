...
Ini **bukan** aktivasi BR-009 §3.4 yang sama — §3.4 mensyaratkan minimal satu Case (`ada Case CLOSED yang relevan`), sedang jalur ini nol Case. Kode menyebutnya "Thin BR-009 lab path... (BQ-011)" — mengacu ke `BR-009 — Complaint Closure` (`02 Business Rules/…v1.0.md`, bagian `# BR-009 — Complaint Closure` → **Preconditions #3**: "Minimal satu Resolution final pada Case yang relevan, atau pengecualian beralasan (Complaint dibatalkan sebelum Case — jarang)") dan `BQ-011` (tabel "BQ | Mode A policy (LOCKED)": "D-02 retained (no Case-at-intake); timing after REGISTERED = BQ-002") sebagai basis yang mengizinkan Complaint tanpa Case sejak awal — bukan `FR-004` (itu Attachment Upload di FRD Batch 1, tidak relevan di sini).

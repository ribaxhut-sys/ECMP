...
**Predikat baca kanonik (2026-08-13, lanjutan M-026-3):** setelah reader jadi Aggregate-only, kosakata `OPEN`/`ESCALATED` masih terpecah empat (dashboard 2 disposisi, filter list 6, counter Users directory 4, donut report 1). Disatukan di `backend/app/modules/cm_batch1/predicates.py`: `OPEN` = bukan `CLOSED` (sesuai DEC-025 M-025-1, sehingga open+closed=total); `ESCALATION_ACTIVE` = `ESCALATE_PENDING_APPROVAL|ESCALATE_APPROVED|HQ_SCHEDULED` untuk KPI/dashboard **summary** dan donut `/reports/by-status`; `ESCALATION_FAMILY` (6 nilai) untuk drill-down "pernah eskalasi". **Koreksi B2 (2026-08-13):** donut `/dashboard/aggregate-kpis` (`cm_batch1_activity_provider.py`, sumber nyata FE — bukan `/reports`) memakai partisi terpisah yang sengaja **tidak** disatukan ke `ESCALATION_ACTIVE`; lihat catatan B2 di bawah. Kontrak wire `/reports` **tetap** memakai label enum Foundation — mapping-nya saja yang dibetulkan (`status` memutus lebih dulu; `ASSIGNED` tidak lagi diterbitkan karena CM tidak punya state itu). Perpindahan kosakata kontrak report ke `REGISTERED|IN_PROGRESS|CLOSED` **belum** diputuskan.

**B2 — tutup lubang predikat (2026-08-13):** dua pemanggil lolos dari sapuan pertama. (1) `cm_batch1_activity_provider.py` (sumber FE `/dashboard/aggregate-kpis`, dipakai `loadDashboardData.ts` dan `loadReportsData.ts` — bukan `/reports/by-status`) — `open`/`closed` kini `predicates.CLOSED_STATUS`; donut mutually-exclusive-nya (`escalatePending`/`waitingAssignment`/`escalateApproved`) **sengaja tidak disentuh** — itu partisi REGISTERED-only yang berbeda by design, bukan definisi "escalated" — jadi `HQ_SCHEDULED` masih jatuh ke `waitingAssignment`; menambah slice ke-4 adalah keputusan kontrak FE terpisah. (2) `Batch1Store.work_stats_for_user` (jalur in-memory Mode A) memakai keluarga 4-nilai lama (tanpa `HQ_SCHEDULED`/`RETURNED_TO_BRANCH`) padahal `CmBatch1Repository.work_stats_for_user` (jalur SQL) sudah `ESCALATION_FAMILY` 6-nilai sejak commit sebelumnya — disamakan. `predicates.OPEN_STATUSES` (tuple statis, tak terpakai, bertentangan dengan `is_open()`) dihapus — satu definisi, bukan dua. Tes baru: `test_store_work_stats_matches_sql_family_not_the_old_4value_set` mengunci paritas store↔SQL.

...
| TC-CM-FR001-05 | Unauthorized → reject + security audit | AC-CM-FR001-05 | API-500 | — | Must | **Automated** | `backend/tests/test_cm_batch1.py::test_tc_cm_fr001_05_unauthorized_create_rejected` (HTTP 403 + FORBIDDEN, any environment) + `test_tc_cm_fr001_05_unauthorized_create_writes_security_audit` (`security.permission_denied` audit row; `@requires_postgres`, skips without a live Postgres — same gate as `test_secmig_p5_security_smoke.py`) |
...
| TC-CM-FR001-08 | Strict mode + Master Customer down → reject | AC-CM-FR001-08 | API-500 | — | Must | **Automated** | `backend/tests/test_cm_batch1.py::test_tc_cm_fr001_08_strict_master_unavailable_create_rejected` — confirms/locks the customer while Master Customer is reachable, then flips `StubCustomerProvider._available = False` before `create_complaint`, asserting the existing `service.py` reject at the `self._customers.exists(...)` re-check (~L723) fires |
| TC-CM-FR001-09 | Notification down → Complaint remains; outbox records failure | AC-CM-FR001-09 | API-500 | EVT-CM-005 | Must | **Blocked (not a test gap)** | EVT-CM-005 (`NotificationOutboxEnqueued`) is catalogued **Planned** in `08 Event Catalog/events/events.yaml` — no notification-enqueue call exists anywhere in `create_complaint`/`event_factory.py`/`side_effects.py`, and the platform `app.modules.notification` module is not wired into CM Batch-1 at all. There is no behavior to test yet; writing one now would either be vacuous or require implementing the EVT-CM-005 integration first (out of scope — no new business logic per this batch's constraints). Re-open once EVT-CM-005 ships. |
